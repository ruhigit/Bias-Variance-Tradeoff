function[avg_test_error,final_opt_lamda]=linear_kernel(da,la)
...
for t=1:3
   ...
   %%% for each value of lamba calculate error
        for l=1:length(lamda)
            k_small=zeros(length(validation_x),1);
            for va=1:length(validation_x)
                i1=1:1:size_data;
                k_small(i1,1)=(kernel_fun(x(i1,:),validation_x(va,:)));
    ...
    I=eye(size_trdata);
     K_=zeros(size_trdata,size_trdata);
        ...
        y_hat_=zeros(length(te_data),1);
         k_small_=zeros(length(te_data),1);
        for va=1:length(te_data)
            i1=1:1:size_trdata;
            k_small_(i1,1)=(kernel_fun(tr_data(i1,:),te_data(va,:)));
